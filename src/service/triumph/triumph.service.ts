...
const debug = debugModule('api:service:triumph');
class TriumphService {
	...
	public async upload(filesList: any, comanyId: number, isBrokerage: boolean = false): Promise<any> {
		...
		const companyIntegrationDetails = await this.companyService.getCompanyIntegration(comanyId, 'Factor', 'Triumph', isBrokerage)
		...
		if (companyIntegrationDetails.data && companyIntegrationDetails.data.length > 0) {
			...
			return new Promise<void>((resolve, reject) => {
				this.sftp
					...
					.catch((error: Error) => {
						reject({ message: `Failed to Upload Files  to Triumph Please contact admin ${error.message}` })
					}).finally(() => {
			...
			deleteFiles(filesList.map((file: any) => file.filePath))
			throw { message: 'Company not integrated with Triumph' }
		}
	}
}

export default TriumphService;